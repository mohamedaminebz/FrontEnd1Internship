...
import { createTask, deleteTask, GetTasksByDepartment, updateTask } from '../../Service/task.service';
import { GetPersonByDepartment } from '../../Service/person.service';
...
function ManagerListTasks() {
    ...
    const { data: Tasks, isLoading } = useQuery('Tasks', GetTasksByDepartment, {
        retry: false,
    ...
    const { data: developers } = useQuery('developers', GetPersonByDepartment, {
        retry: false,
        refetchOnWindowFocus: false
    });
    ...
    const textEditor = (options) => {
        ...
        } else if (options.field === 'endDate' || options.field === 'creationDate') {
            return (
    ...
    const handleSubmit = (event) => {
        ...
        if (!hasError) {
            newTask.creationDate = moment(newTask.creationDate).format('YYYY-MM-DD');
    ...
    const deleteTaskDialogFooter = (
        <>
            <Button label="Cancel" icon="pi pi-times" className="p-button-text" onClick={() => setDeleteTaskDialog(false)} />
            <Button
                label="Delete"
                icon="pi pi-trash"
    ...
    return (
        <div>
            ...
            <Dialog header="Add Task" visible={addTaskDialogVisible} onHide={() => setAddTaskDialogVisible(false)} style={{ width: '50%' }}>
                    ...
                    <div className="p-fluid">
                        <div className="p-field-wrapper">
                            <div className="p-field">
                                ...
                                <span className="error-message">{titleError && 'Please enter a Title'}</span>
                            </div>
                            <div className="p-field">
                        ...
                        <div className="p-field-wrapper">
                            <div className="p-field">
                                ...
                                <Calendar
                                    ...
                                    className={startDateError && !newTask.creationDate ? 'p-invalid' : ''}
                                    error={startDateError}
                                    helperText={startDateError ? 'Please enter a Start Date*' : ''}
                                    onChange={(event) => {
                                        setnewTask({ ...newTask, creationDate: event.value });
                                        setstartDateError(false);
                                    }}
                                />
                                <span className="error-message">{startDateError && 'Please enter a Creation Date*'}</span>
                            </div>
                            <div className="p-field">
                                ...
                                />
                                <span className="error-message">{endDateError && 'Please enter a Expected End Date*'}</span>
                            </div>
                        </div>
                        <div className="p-field-wrapper">
                            <div className="p-field">
                                ...
                                <Dropdown
                                    ...
                                    placeholder="Select a project"
                                />
                                <span className="error-message">{projectError && 'Please select a Project*'}</span>
                            </div>
                            <div className="p-field">
                                ...
                                />
                                <span className="error-message">{assignToError && 'Please select a Person*'}</span>
                            </div>
                        </div>
                    </div>
...
export default ManagerListTasks;
